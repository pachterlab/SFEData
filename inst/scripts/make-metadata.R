...
metadatas$seurat <- data.frame(
    Title = c("Seurat Visium mouse brain dataset from SeuratData",
              "Seurat Visium HD mouse brain data subset",
    ...
    Description = c("Subset of stxBrain.SeuratData with the first 50 genes",
                    "Subset of mouse brain Visium HD data with the first 50 genes",
    ...
    BiocVersion = "3.19",
    Genome = c(rep("GRCm38",4), rep("GRCh38",2)),
    SourceType = "RDS",
    ...
    SourceVersion = "",
    Species = c(rep("Mus musculus",4), rep("Homo sapiens",2)),
    TaxonomyId = c(rep("10090",4), rep("9606",2)),
    Coordinate_1_based = NA,
    DataProvider = c("Satija Lab", rep("10X Genomics",3), rep("scOpenLab DKFZ",2)),
    Maintainer = "Alik Huseynov <alikhuseyno@gmail.com>",
    RDataClass = "Seurat",
    DispatchClass = "Rds",
    RDataPath = file.path("SFEData", c("seu_anterior1_visium.rds", "seu_vishd_multi.rds",
